...
-- 		IF(SUM(campaigns_report.`orders`) IS NULL,0,SUM(campaigns_report.`orders`)) orders
		...
		(SELECT DATE_FORMAT(DATE_ADD(campaigns_report.`site_local_date`,INTERVAL -1 day),"%Y-%m-%d") snapshot_date,
...
-- 		SUM(campaigns_report.`attributed_units_ordered1d`) orders
	...
	ELSEIF shopAdDataOverviewType = 'adgroups' THEN
		...
		FROM
		adDateView_day LEFT JOIN 
		(SELECT DATE_FORMAT(DATE_ADD(adgroups_report.`site_local_date`,INTERVAL -1 day),"%Y-%m-%d") snapshot_date,
	...
	ELSEIF shopAdDataOverviewType = 'productads' THEN
		...
		FROM
		adDateView_day LEFT JOIN 
		(SELECT DATE_FORMAT(DATE_ADD(productads_report.`site_local_date`,INTERVAL -1 day),"%Y-%m-%d") snapshot_date,
	...
	#找到这段时间的广告keywords
		...
		FROM
		adDateView_day LEFT JOIN 
		(SELECT DATE_FORMAT(DATE_ADD(keywords_report.`site_local_date`,INTERVAL -1 day),"%Y-%m-%d") snapshot_date,
